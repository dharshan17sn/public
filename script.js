document.addEventListener("DOMContentLoaded", function () {
    ...
    function drawRoute(routePoints) {
      ...
      // Simplify the route by reducing the number of points (optional)
      const simplifiedPoints = routePoints.filter((point, index) => index % 2 === 0); // Example: keep every other point
  
      // Add the new route
      routingControl = L.Routing.control({
        waypoints: simplifiedPoints.map((coords) => L.latLng(coords[0], coords[1])),
